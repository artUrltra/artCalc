    ...
    <div class="jumbotron">
        ...
        <div id="form">
            ...
            <select id="ctype3" class="ctype" style="display:none;">
                {foreach from=$cats item=$i}{if $i['cview'] eq 3}
                    <option value="{$i['cid']}">{$i['cname']}</option>{/if}{/foreach}
            </select>
            <select id="ctype4" class="ctype" style="display:none;">
                {foreach from=$cats item=$i}{if $i['cview'] eq 4}
                    <option value="{$i['cid']}">{$i['cname']}</option>{/if}{/foreach}
            </select>
            <select id="ctype0" class="ctype" style="display:none;">
                {foreach from=$cats item=$i}{if $i['cview'] eq 0}
                    <option value="{$i['cid']}">{$i['cname']}</option>{/if}{/foreach}
            </select>
            <br/>
            <div class="row">
            ...
            <table class="table table-hover" id="result">
                ...
                {foreach $items as $item name=items}
                    <tr id="delete{$item['id']}">
                        <td>{if $item['cview'] eq 1}Раздвижная{elseif $item['cview'] eq 2}Складная{elseif $item['cview'] eq 3}Распашная дверь{elseif $item['cview'] eq 4}Мобильная перегородка{elseif $item['cview'] eq 0}Стационарная перегородка{/if}</td>
                        <td>{$item['cname']}</td>
                        <td>{$item['name']}</td>
