...
function sendUrl(screen) {
    ...
    for (i = 1; i <= TOTAL_PAINTING; i++) {
        ...
        TotalPolotno2 = TotalPolotno2 + parseInt(s);
    }
    var AddonTotal2 = 0;
    for (i = 1; i <= TOTAL_PAINTING; i++) {
        AddonTotal2 = AddonTotal2 + parseInt($("#state2").contents().find('*[data-id="' + i + '"]').find(".data-price-in-napolnenii").text());
    }
    var FurnituraTotal2 = $("#state2").contents().find('.furnitura-price .price').text();

    var TotalPolotno3 = 0;
    for (i = 1; i <= TOTAL_PAINTING; i++) {
        var s = $("#state3").contents().find('*[data-id="' + i + '"]').find(".data-price-in-profil").text();
        TotalPolotno3 = TotalPolotno3 + parseInt(s);
    }
    var AddonTotal3 = 0;
    for (i = 1; i <= TOTAL_PAINTING; i++) {
        AddonTotal3 = AddonTotal3 + parseInt($("#state3").contents().find('*[data-id="' + i + '"]').find(".data-price-in-napolnenii").text());
    }
    var FurnituraTotal3 = $("#state3").contents().find('.furnitura-price .price').text();
    ...
    var url = '?furnitura-select=' + furnituraSelect +
        ...
        '&selectedKarkas=' + m2 +
        '&state1Profile=' + TotalPolotno +
        '&state1Furnitura=' + FurnituraTotal +
        '&state1Zapolnenie=' + AddonTotal +
        '&state1Zamer=' + '1000' +
        '&state1Montaj=' + '10000' +
        '&state1Dostavka=' + '1000' +
        '&state2Profile=' + TotalPolotno2 +
        '&state2Furnitura=' + FurnituraTotal2 +
        '&state2Zapolnenie=' + AddonTotal2 +
        '&state2Zamer=' + '1000' +
        '&state2Montaj=' + '10000' +
        '&state2Dostavka=' + '1000' +
        '&state3Profile=' + TotalPolotno3 +
        '&state3Furnitura=' + FurnituraTotal3 +
        '&state3Zapolnenie=' + AddonTotal3 +
        '&state3Zamer=' + '1000' +
        '&state3Montaj=' + '10000' +
        '&state3Dostavka=' + '1000';
    url = url.replace(/°/g, "");
