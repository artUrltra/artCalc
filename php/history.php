...
if ($_REQUEST["action"] != "apply") {
    ...
    while ($result = $results->fetchArray(SQLITE3_ASSOC)) {
        ...
        echo '<td><button onclick=\'historyAjaxApply("apply", "{\"comment\":'.$result["comment"].'}")\'>Apply</button></td>';
    ...
    $results = $db->query("SELECT * FROM variants WHERE comment = ".$jsObj["comment"]);
